...
- 子弹追踪
- 平面内找任意点
- 19号中午去华阳街道买了王妈手撕兔和兔头，准备过年带回去，2022年虽然有点惨，但是回家该有的礼节还是要有的。
...
- (已处理)：独立出技能系统，后续战斗这块整体架构即： AI+技能+buff+子弹
...
- 战斗结束特效集成
- 战斗UI血条等大小和相机距离建立关系
- (已处理)：总结一下使用NavmeshAgent不使用ObstacleAvoidance如何避免出现拥挤。以及解决Carve导致的诡异现象
...
## 20230227 星期一
- (已处理：事件监听未移除导致的)：OnPlayerDispatchTroopsTimeAdd会出现player为null的情况，很奇怪。需要查一下
- TODO：ECS、DOTS学习:https://auniquepig.com/2021/06/21/DotsSwords/

- 出新包要注意的地方：1，Config表关闭debug、关闭shortcut 2，移除DebugReporter.txt 3，cfg关闭日志trace和打印 4，检查备份代码可否被删除


## 20230228 星期二
- (已处理：协议头使用的消息长度字段为uint，结算时数据量会大于uint的最大值)：部分玩家直播间世界排行榜没弹出
