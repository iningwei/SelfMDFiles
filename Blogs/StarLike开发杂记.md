...
```
res version:1
1，未入场情况下b1 b+任意数字可入场
2，b1 b2 b3 b4 b5 改成可以爆光能量条


res version:2
1，增加自销毁机制
2，设置界面独立成窗体，在结算界面也会显示
3，输入B/b会随机为b1 b2 b3 b4 b5
4，回合时间动态扩容，防止小兵持续积累
...
```
res version:4
1，结算根据胜负情况，积分加成（系数1.2）
2，结算排行榜未显示世界排名信息
3，Z列表自动滚动
4，子弹表增加伤害忽略字段（核弹炸不到基地）
5，魅影集成
6，仙女棒提示bug
7，首次送礼召唤魅影
8，礼物兵price*30
...
res version：8
1，普通兵打礼物兵的伤害得分除以10
2，落座其它直播间停止能量增长和击杀、伤害积分
3，内网测试公告功能
4，TODO：取消白名单，通过uid和房间号登录
5，一些兵种、特效资源的重做
6，恢复电池上限
```
```
...
res version：10
1，z快捷键功能默认显示
2，取消礼物弹幕同时间戳的数据去重，会导致弹幕丢失
3，新加一些参数
```
```
res version：11
1，解决bug: onKickPlayer之前未加Player是否为空的判定，服务器却下发了一个不存在的玩家id过来导致了空引用。
```
```
res version：12
1，氛围兵
2，TODO bug：玩家排名问题。分数高的排名反而低
```

## 20230809-20230812 星期三-星期六
res version：13
1，积分排序还是要加上相等返回0。不然会报错。不知道之前为啥没有测试出来呢。。。
...
## 20230814-20230815 星期一-星期二
res version：18
1，一些数值更新
2，巡洋舰不攻击bug(dis判断未减除radius)

res version：19
1，guard dis判定未加上玩家获得到的额外攻击距离
