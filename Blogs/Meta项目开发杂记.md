...
### 2022/12/02
1，单独chunk烘焙光照贴图，新信息未写入map

### 2022/12/03
（已处理）1，map数据导出json方便后续核对
2，服务端关服等情况下强制踢人的处理（场景销毁等）
3，UWA学堂收藏了几个好的学习资料，TODO：

### 2022/12/04
（已处理）1，角色表情BlendShapes测试
（已处理）2，joystick优化,拖拽按钮转向和拖动方向一致
...
### 2022/12/09
（已处理）1，初步处理了一下断线重连
2，经过测试对base场景烘焙后，再动态加载的地图substractive就生效了。
但是dc升高了，明天用光武的机器对比一下

### 2022/12/10
``周六加班``
1，处理战斗模块的问题
梭理记分逻辑、记分不准问题、双按钮点击消失问题
2，单击要改成按下计算分数。长按则按照按下和松开两种状态
...
### 2022/12/21
一些问题：
1， "110@103", "110@105"；两个下行空消息未添加，未处理
